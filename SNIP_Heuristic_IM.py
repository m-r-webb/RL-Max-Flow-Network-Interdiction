...
env_multiple_interdiction = True
env_max_budget = 25
...
def run_episode(episode):
    ...
    while budget >= 3:
        ...
        mask = (obs['edge_interdicted'] == 0) & (obs['edge_interdiction_probability'] != 0) & (obs['edge_costs'] <= budget)
        masked_actions = np.where(mask, actions, 0)
 ...
 #       if action != env.get_attr("num_interdictable_edges")[0] and action.item() < 270:
        action_key = eval_env.interdictable_edges[action]
...
if __name__ == '__main__':
    ...
    np.save('max_flow_vals_M.npy', max_flow_vals)
    np.save('optimal_obj_vals_M.npy', optimal_obj_vals)
    np.save('optimal_obj_vars_M.npy', optimal_obj_vars)
    np.save('agent_best_rewards_M.npy', agent_best_rewards)

    # Save frozenset list as .pkl file
    with open('all_optimal_interdiction_edges_M.pkl', 'wb') as f:
        pickle.dump(all_optimal_edges, f)
    with open('all_agent_actions_M.pkl', 'wb') as f:
