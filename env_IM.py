...
class CustomEnv(gym.Env):
    def __init__(self, nodes, edges, deterministic_agent=True, fixed_costs=True, curriculum_training=False, initial_budget = None, 
        ...
        self.max_num_edges = self.num_interdictable_edges #168 #270 # #1140 #651
        self.max_num_bidirectional_edges = self.num_bidirectonal_edges #56 #180 # #760 #422
        self.max_num_nodes = self.sink_nodes[0] #66 #102 #402 #227
    ...
    def solve_max_flow(self):  
        ...
        # Update Variable Upper Bounds  #IM CHANGE
        self.maxflow_model.setAttr("UB", [self.flow_var[e] for e in self.interdictable_edges],
                                   np.random.binomial(1, (1-self.state["edge_interdiction_probability"][0:self.num_interdictable_edges])**self.state["edge_interdicted"][0:self.num_interdictable_edges])*self.state["edge_capacity"][0:self.num_interdictable_edges])  #np.random
    ...
    def solve_stochastic_max_flow(self, n_scenarios = 50, seed = 173, interdicted_edges = []):
        ...
        if hasattr(self, 'stochastic_aabg_constr'):
            ...
            self.optimal_stochastic_model.update()  # Force model synchronization
            del self.stochastic_aabg_constr
    ...
    def solve_stochastic_max_flow_IM(self, n_scenarios = 50, seed = 173, interdicted_edges = [], interdicted_quantities =[]):
        ...
        if hasattr(self, 'stochastic_aabg_constr_IM'):
            ...
            self.optimal_stochastic_model_IM.update()  # Force model synchronization
            del self.stochastic_aabg_constr_IM
